...
| 文件 | 行数 | 职责 |
|---|---|---|
| `src/tts/tts-manager.ts` | 1128 | 核心：TTSChunk 定义、TTSPlaybackCallbacks、三引擎类（WebSpeech 内嵌 L79 / Kokoro 基类 L330 / Browser 子类 L604 / Server 子类 L617）、TTSManager 调度器（预生成缓冲池、流水线、作废机制、自动翻章配合） |
| `src/tts/zipvoice-engine.ts` | 551 | 浏览器推理引擎：sherpa-onnx wasm 的 **Worker 池**（本次 C 改造）、模型加载、任务调度、超时/错误处理 |
| `src/tts/server-engine.ts` | 103 | 服务端推理 HTTP 客户端：`synthesizeServer`（SSE/轮询取音频）、`cancelServerInference`（取消排队请求） |
| `src/tts/tts-cache.ts` | 241 | IndexedDB 模型资源缓存（412MB：model.onnx 310MB + voices + espeak + jieba dict） |
...
| `src/hooks/useAudioPlayer.ts` | 475 | 播放 Hook：manager 生命周期、回调→store 桥接、自动翻章、seek、位置记忆 |
| `src/components/tts/AudioPlayer.tsx` | 371 | 朗读栏 UI：播放控制、进度、**预生成提示/缓冲水位/立即播放**（本次 A 改造） |
| `src/components/settings/TTSSettings.tsx` | 781 | 设置页：引擎选择、音色试听、**预生成段数/Worker 数**（本次新增） |

测试文件（8 个）：`tts-manager-pipeline`（8 例，缓冲池核心）、`tts-manager`、`tts-store`（含新增持久化例）、`useAudioPlayer-autonext`（5 例）、`useAudioPlayer-position`、`normalize-text`、`paragraph-tracking`、`text-preprocess`、`tts-preload`、`voice-classify`。
...
- **加载**：`loadModel` 串行初始化池（逐个 worker init，避免内存峰值叠加）；每个 worker 用 `files.slice(0)` 拷贝 + transfer（零拷贝传输）
- **释放**：`resetWorker` 终止整个池、清空队列、reject 全部 pending
- **配置**：`setWorkerPoolSize(n)`（1-3），朗读前应用（模型未加载时生效，UI 注明"修改后下次朗读生效"）
...
当前全量：**524 测试通过**，`tsc -b --force` 全绿，`vite build` 成功。
...
3. **workerCount 生效时机**：模型已加载时调整不会即时生效（下次朗读应用），UI 已注明
...
7. **worker 崩溃**：单 worker onerror 快速失败并 reject 全部 pending（沿用现状），池化后未做自动重建（失败会走引擎降级到 Web Speech）
